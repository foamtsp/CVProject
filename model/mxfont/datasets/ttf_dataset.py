...
class TTFTrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(font, char)
        trg_img = self.transform(np.array(render(font, char)))
        ...
        style_imgs = torch.stack([self.transform(np.array(render(font, c))) for c in style_chars])
        ...
        char_imgs = torch.stack([self.transform(np.array(render(self.key_font_dict[k], char))) for k in char_keys])
...
def load_data_list(data_dir, char_filter=None):
    ...
    for font_path in font_paths:
        ...
        with open(str(font_path).replace(".ttf", ".txt"), 'r',encoding='utf-8') as f:
            chars = f.read()
